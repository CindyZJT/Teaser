...
def set_metrics(pl_module):
    #print(pl_module.transformerSQHparams.config["loss_names"])
    for split in ["train", "val"]:
        for k, v in pl_module.transformerSQHparams.config["loss_names"].items():
            if v < 1:
                continue
...
def epoch_wrapup(pl_module):
    ...
    if pl_module.transformerSQHparams.config["get_recall_metric"] and not pl_module.training:
    ...
    for loss_name, v in pl_module.transformerSQHparams.config["loss_names"].items():
        if v < 1:
            continue
...
def set_task(pl_module):
    pl_module.current_tasks = [
        k for k, v in pl_module.transformerSQHparams.config["loss_names"].items() if v >= 1
    ]
    #print("set_task", pl_module.current_tasks)
    return

def set_schedule(pl_module):
    lr = pl_module.transformerSQHparams.config["learning_rate"]
    wd = pl_module.transformerSQHparams.config["weight_decay"]
    ...
    lr_mult = pl_module.transformerSQHparams.config["lr_mult"]
    end_lr = pl_module.transformerSQHparams.config["end_lr"]
    decay_power = pl_module.transformerSQHparams.config["decay_power"]
    optim_type = pl_module.transformerSQHparams.config["optim_type"]
    ...
    warmup_steps = pl_module.transformerSQHparams.config["warmup_steps"]
    if isinstance(pl_module.transformerSQHparams.config["warmup_steps"], float):
