...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, masked=False, fowd_range=None):
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale #(batch_size, head_num, seq_len, seq_len)
        if masked:
            seq_mask = torch.triu(torch.ones(N, N), diagonal=1).to(device)
            seq_mask = seq_mask.bool()
            attn = attn.masked_fill(seq_mask[None, None, :, :], float("-inf"))
        if fowd_range!=None:
            seq_mask = torch.zeros(N, N).to(device)
            seq_mask[:fowd_range, :fowd_range] = torch.triu(torch.ones(fowd_range, fowd_range), diagonal=1)
            seq_mask[fowd_range:, 1:fowd_range] = 1
            seq_mask = seq_mask.bool()
            attn = attn.masked_fill(seq_mask[None, None, :, :], float("-inf"))
        if mask is not None:
            #print("viT_mask", mask.size())
            mask = mask.bool()
...
class Attention2(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, masked=False, fowd_range=None, layers=None):
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        B, N, C = q.shape
        q = (
            self.q(q)
        ...
        k = (
            self.k(k)
            .reshape(B, M, 1, self.num_heads, C // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )
        v = (
            self.v(v)
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale #(batch_size, head_num, M, N)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, m, mask=None, fowd_range=None, layers=None):
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        B, N, C = x.shape
        q = (
            self.q(x)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, header, x):
        #print(x.size())
        #print(self.pe.repeat(x.size()[0], 1, 1).size())
        pos_emb = self.pe.repeat(x.size()[0], 1, 1)
        seq = torch.cat([header+pos_emb[:,0:1,:], self.dropout(x+pos_emb[:,1:,:])], dim=1).float()
        return seq
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, fowd_range=None):
        _x, attn = self.attn(self.norm1(x), mask=mask, masked=False, fowd_range=fowd_range) 
...
class Block2(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, fowd_range=None):
        v = self.pre_dropout(v)

        _x, attn = self.attn1(self.norm1(q), mask=mask, masked=False, fowd_range=fowd_range) 
        x = q + self.drop_path(_x)
        _x, attn = self.attn2(self.norm_q(x), self.norm_k(k), self.norm_v(v)) 
        x = x + self.drop_path(_x)
        #_x, attn = self.attn(self.norm_q(q), self.norm_k(k), self.norm_v(v))
        #x = q + self.drop_path(_x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x, attn
...
class Block_Decoder(nn.Module):
    ...
    def forward(self, x, m, mask=None):
        _x, attn_1 = self.attn(self.norm1(x), mask=mask, masked=True)
        x = x + self.drop_path(_x)
        #print("x_1", _x.cpu().numpy())
        _x, attn_2 = self.c_attn(self.norm2(x), m)
        #print("x_2", _x.cpu().numpy())
        x = x + self.drop_path(_x)
        x = x + self.drop_path(self.mlp(self.norm3(x)))
        return x, attn_2        
"""
class Block_Topic(nn.Module):
    def __init__(
        self,
        dim,
        num_heads,
    ...
    def forward(self, q, k, v):
        _x, attn = self.c_attn(self.norm1(q), self.norm1(k), self.norm1(v))
        x = x + self.drop_path(_x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x, attn        
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks_topic = nn.ModuleList(
            [
                Block2(
                    dim=embed_dim,
                    num_heads=num_heads,
        ...
        """
        """
        self.blocks_sent = nn.ModuleList(
            [
                Block(
                    dim=embed_dim,
                    num_heads=num_heads,
        ...
        """       
        """
        self.sent_decoder = nn.ModuleList(
            [
                Block_Decoder(
                    dim=embed_dim,
                    num_heads=num_heads,
        ...
        self.sent_decoder.apply(objectives.init_weights)
        """
        """
        ...
        self.text_decoder.apply(objectives.init_weights)
        """
        self.norm = norm_layer(embed_dim)
        self.norm_imageQuery = norm_layer(embed_dim)
        self.norm_sent = norm_layer(embed_dim)
        self.norm_topic = norm_layer(embed_dim)
...
def _create_vision_transformer(variant, pretrained=False, distilled=False, **kwargs):
    ...
    if pretrained:
        ...
        """
        blocks_pretrained = model.blocks.state_dict()
        blocks_sent_dict = model.blocks_sent.state_dict()
        #for k, v in blocks_pretrained.items():
        #    print(k)
        pretrained_dict = {k: v for k, v in blocks_pretrained.items() if k in blocks_sent_dict}
        blocks_sent_dict.update(pretrained_dict)
        model.blocks_sent.load_state_dict(blocks_sent_dict)
        """
    return model
